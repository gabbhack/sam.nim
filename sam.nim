...
proc findValue(m: Mapper, key: string, start = 0): int {.inline.} =
  result = -1
  var
    i = start + 1
    tok: JsmnToken
  let
    endPos = m.tokens[start].stop
    tokens = m.tokens

  if m.tokens[start].kind != JSMN_OBJECT:
    raise newException(ValueError, "Object expected " & $(m.tokens[start]))
